...
**Tecnología.** React con Vite y `react-markdown` en el frontal (con el historial en `localStorage`); FastAPI, LangGraph y FAISS en el backend, con embeddings generados a través de la API de Gemini (`gemini-embedding-001`); Gemini (`gemini-3.5-flash-lite`) como modelo de lenguaje. Los embeddings se probaron primero con un modelo local (`multilingual-e5-small`, vía `sentence-transformers`/PyTorch), pero se sustituyeron por la API de Gemini al comprobar que la carga de PyTorch no era viable con la memoria disponible en el plan gratuito de Render; se verificó en el panel de uso de Google AI Studio que el margen gratuito de `embed_content` (1.000 peticiones/día) es suficiente para el volumen de uso esperado. Docker Compose para el entorno local, con backend en Render (contenedor Docker) y frontend en Vercel (build nativo de Vite) en producción.
